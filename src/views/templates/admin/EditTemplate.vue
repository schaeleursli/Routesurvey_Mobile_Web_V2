<template>
    <div class="template-editor-page">
                ...
                <BasePanel class="editor-panel" elevation="level1" :scrollable="true">
                    <template #header>
                        <div class="header-actions-group">
                            <input v-model="editName" class="editor-title-input" :placeholder="$t('templateTitle')" />
                ...
                <div class="modal-body">
                    <form @submit.prevent>
                        ...
                        <div class="media-upload-section">
                            ...
                            <div class="media-preview" v-if="editMedia.length > 0">
                                <div v-for="(media, index) in editMedia" :key="index" class="media-item">
                                    ...
                                    <button type="button" class="remove-media" @click="removeEditTemplateMedia(index)">
                                        <i class="bi bi-x"></i>
                                    </button>
                                </div>
                            </div>
                        ...
                        <div class="checkbox-group">
                            ...
                            <BaseFormField type="checkbox" :label="$t('isDefault')" v-model="editIsDefault" />
                        </div>
                    </form>
                </div>
        ...
        <BaseConfirmationModal :visible="showUnsavedChangesModal" :title="t('unsavedChanges')"
            ...
            :danger-text="t('leaveWithoutSaving') || 'Leave Without Saving'" :primary-text="t('stay')"
...
const initialMetadataState = ref(null);
...
// Helper to check for metadata changes
const hasMetadataChanges = () => {
    if (!initialMetadataState.value) return false;
    
    // Compare basic fields
    if (editName.value !== initialMetadataState.value.name) return true;
    if (editDescription.value !== initialMetadataState.value.description) return true;
    if (editPrice.value !== initialMetadataState.value.price) return true;
    if (editIsVisible.value !== initialMetadataState.value.isVisible) return true;
    if (editIsDefault.value !== initialMetadataState.value.isDefault) return true;
    
    // Compare media (simple length check + url check)
    if (editMedia.value.length !== initialMetadataState.value.media.length) return true;
    // Deep check if needed, but length + first item url usually sufficient for this level, 
    // strictly speaking we should check all.
    const currentMediaJson = JSON.stringify(editMedia.value);
    const initialMediaJson = JSON.stringify(initialMetadataState.value.media);
    if (currentMediaJson !== initialMediaJson) return true;

    return false;
};

const hasUnsavedChanges = () => {
    return hasChanges.value || hasMetadataChanges();
};

const captureInitialMetadata = () => {
    initialMetadataState.value = {
        name: editName.value,
        description: editDescription.value,
        price: editPrice.value,
        isVisible: editIsVisible.value,
        isDefault: editIsDefault.value,
        media: JSON.parse(JSON.stringify(editMedia.value))
    };
};
...
onMounted(async () => {
    ...
    try {
        ...
        if (templateId) {
            ...
            if (template) {
                // Load template structure from content
                ...
                if (template.content) {
                    try {
                        ...
                        // Content is not JSON, treat as legacy format
                        console.log('Template content is not JSON, initializing default structure');
                    }
                }
                ...
                if (templateStructure && templateStructure.structure) {
                    ...
                    reportSections.value = templateStructure.structure.sections || [];
        ...
        showMessage({ status: 'error', message: 'Error loading template' });
        router.push({ name: 'ManageTemplates' });
    } finally {
...
const handleSaveTemplate = async () => {
    ...
    try {
        ...
        // Update editContent with JSON structure
        editContent.value = JSON.stringify({
            structure,
            version: '2.0',
